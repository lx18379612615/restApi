...
log = Logger(logname="app.log", logger="spread").getlog()
...
def get_main_feature():
    ...
    log.info("request body param: {}".format(body))
    log.info("request tag param: {}".format(tag))
    log.info("request group_id param: {}".format(group_id))
    log.info("request num param: {}".format(num))
    log.info("request status param: {}".format(status))
    ...
    try:
        ...
        if df.empty:
            ...
            error_response = json.dumps(DataResponse.DataResponse('', 1000, '客群有效数据集为空').__dict__,
                                        ensure_ascii=False)
            return error_response
    except Exception as e:
        log.error("get data from es ERROR:", e)
        log.error(traceback.format_exc())
        error_response = json.dumps(DataResponse.DataResponse('', 1001, '获取客群数据超时，请减少客群数据大小').__dict__,
                                    ensure_ascii=False)
        return error_response
    ...
    log.info("result of local_get_main_feature(df): {}".format(json.dumps(result)))
...
def local_get_main_feature(x_train, named_cols):
    ...
    while i < len(position):
        ...
        x_return[j] = named_cols[position[i]] + ":" + str(max_num) + ":" + str(percent) + ":" + str(
            start_value) + ":" + str(end_value)
...
def population_spread():
    ...
    # todo body里面includes的标签是哪里来的，有些什么标签
    log.info("body: {}".format(body))
    log.info("tag: {}".format(tag))
    log.info("wait_body: {}".format(wait_body))
    log.info("group_id: {}".format(group_id))
    log.info("wait_group_id: {}".format(wait_group_id))
    log.info("num: {}".format(num))
    log.info("status: {}".format(status))
    log.info("percent: {}".format(percent))

    # x_train: 1000个种子用户的标签数据
    ...
    if tag not in x_train.columns:
        ...
        error_response = json.dumps(
            DataResponse.DataResponse('', 3000, '标签列{}数据异常,应该为数字型'.format(tag)).__dict__,
            ensure_ascii=False)
        return error_response
    # y_train: 所有种子用户tag标签数据
    y_train = x_train[tag]
    log.info("y_train")
    log.info(y_train)
    ...
    if x_test.empty:
        error_response = json.dumps(DataResponse.DataResponse('', 1000, '测试集合有效数据集为空').__dict__,
                                    ensure_ascii=False)
        return error_response
    ...
    # todo x_train暂时也把[tag]列去掉
    # x_train.drop([tag], axis=1, inplace=True)
    x_test_wait.sort_index(axis=1, inplace=True)
    ...
    if x_percent_df.columns.__contains__(status):
        while i < y_predict.size:
            if x_percent_df.loc[i, status] >= float(percent):
                # y_predict[i] = 1
                info = {key_field_name: x_test[key_field_name][i], "client_name": x_test["client_name"][i],
                        "mobile_tel": x_test["mobile_tel"][i],
                        "percent": float("%.2f" % (x_percent_df.loc[i, status] * 100))}
                y_percent[j] = info
                j = j + 1
            # else:
            #     y_predict[i] = 0
